...
@nb.jit(nopython=True, fastmath=True)
def von_Newman(ρf, H):
    relax, excit = np.zeros((par.nt,par.nt), dtype = np.complex128), np.zeros((par.nt,par.nt), dtype = np.complex128)
    relax += par.Γc * (par.nth + 1)/2 * (par.a_cre @ par.a_anh @ ρf - 2 * par.a_anh @ ρf @ par.a_cre + ρf @ par.a_cre @ par.a_anh)
    excit += par.Γc *  par.nth /2 *     (par.a_anh @ par.a_cre @ ρf - 2 * par.a_cre @ ρf @ par.a_anh + ρf @ par.a_anh @ par.a_cre)
    return -1j * (H @ ρf - ρf @ H) - (relax + excit)
...
def run_traj(data):
    ...
    for st in range(data.nSteps):
        if (st % par.nskip == 0):               # WRITTING OF THE DENSITY MATRIX | SAVE ONLY THE DIAGONAL ELEMENTS
            ρ = data.ρt
            ρ = ρ.copy()
            data.ρw[iskip,:] = np.real(ρ.reshape(par.nt**2))
            iskip += 1
