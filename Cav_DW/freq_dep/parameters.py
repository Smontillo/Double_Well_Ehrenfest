...
ρR      = np.zeros((nDW,nDW), dtype = np.complex128)
ρR[0,0] = 1.0 + 0 * 1j
# CAVITY IN THERMAL STATE
# ρc      = np.zeros((nc,nc), dtype = np.complex128)
ρc      = np.diag(np.exp(-β * ωc * np.diag(aδ @ a))/(np.sum(np.exp(-β * ωc * np.diag(aδ @ a)))))
# ρc[0,0], ρc[1,1] = 0.95, 0.05
ρ0      = np.kron(ρc, ρR)

# SIMULATION PARAMETERS ==============================
parallel = True                                            # DO PARALLELIZATION
Cpus     = 20                                              # NUMBER THE CPUS USE FOR PARALLELIZATION
NTraj    = 6000                                            # NUMBER OF TRAJECTORIES
