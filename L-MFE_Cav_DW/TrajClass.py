...
spec = [
    ...
    ('ψt',              complex128[:]), # DENSITY MATRIX AT TIME t
    ('ψ',               complex128[:]), # DENSITY MATRIX AT TIME t
    ('p',               complex128[:]), # DENSITY MATRIX AT TIME t
    ('q',               complex128[:]), # DENSITY MATRIX AT TIME t
    ('H',             complex128[:,:]), # DENSITY MATRIX AT TIME t
    ('H_bc',          complex128[:,:]), # ELECTRONIC HAMILTONIAN | DEPENDENT OF THE POSITION OF THE BATH OSCILLATOR
    ('ψw',               float64[:,:]), # PLACE HOLDER FOR THE DENSITY MATRIX
]
...
class trajData(object):
    def __init__(self, nDW, nc, ndof, nSteps, nData):
        ...
        self.ψt     = np.zeros((self.nt), dtype = np.complex128)
        self.ψ      = np.zeros((self.nt), dtype = np.complex128)
        self.p      = np.zeros((self.nt), dtype = np.complex128)
        self.q      = np.zeros((self.nt), dtype = np.complex128)
        self.H      = np.zeros((self.nt,self.nt), dtype = np.complex128)
        self.H_bc   = np.zeros((self.nt, self.nt), dtype = np.complex128)
        self.ψw     = np.zeros((nData,self.nt))