...
freq = [1190] #[800, 900, 1000, 1050, 1100, 1150, 1190, 1200, 1230, 1250, 1350, 1450, 1550]
# LOADING OF DATA
cpus = par.Cpus
Nst = par.nData
...
Lind = np.loadtxt('/scratch/smontill/Simpkins/Cav_DW/lifetime_2ps/Lind_DW.txt')
...
ax.plot(Lind[:,0], Lind[:,3],  ls = '--', lw = 1, color = 'black')
ax.plot(Lind[:,0], Lind[:,4],  ls = '--', lw = 1, color = 'black')
...
ax2 = ax.twinx()
ax2.plot(np.NaN, np.NaN, ls= '--', label= 'Lindblad', c='black')
ax2.plot(np.NaN, np.NaN, ls= '-',label="Eric's Method", c='black')
ax2.get_yaxis().set_visible(False)

ax.legend(title ='Vibrational States', loc=0, frameon = False, fontsize = 9, handlelength=1, title_fontsize = 9, labelspacing = 0.2)
ax2.legend(loc = 6, frameon = False, fontsize = 9, handlelength=2, labelspacing = 0.2)
