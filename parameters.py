...
def Rx(nDW):
    ...
    for j in range(nDW):
        for i in range(nDW):
            avg_pos = diaV[:,j].conjugate() * x * diaV[:,i]
...
def BathParam(λD, γD, N, num):    
    ...
    # Huo. P., et al (Mol. Phys. 2012, 110, 1035–1052)
        ...
        cj[:] = 2 * ωj * (λD * np.arctan(ω_max/γD)/(np.pi * N))**0.5
...
def Hel_cons():
    ...
    H += np.diag(diaE)                                  # VIBRATIONAL STATES ENERGY
...
L = 200.0                                                 # UPPER AND LOWER R0 LIMIT [-L, L]
x = np.linspace(-L,L,N)                                   # R0
dx = x[1] - x[0]                                        
EDW, VDW = DVR(x,M,wDW)                                   # EIGENENERGIES AND EIGENSTATES FOR THE DW 
...
tf       = 5000 * fstoau                                   # SIMULATION TIME IN FEMTOSECONDS
...
nskip    = 1                                               # FRAME SAVED ⇒ HAS NOT BEEN PROGRAM YET, MUST BE 1!!!!

# BATH PARAMETERS ==============================
ndof   = 200                                               # NUMBER OF BATH OSCILLATORS
...
H0   = Hel_cons()                                          # ELECTRONIC HAMILTONIAN | INDEPENDENT OF THE POSITION OF THE BATH OSCILLATOR
